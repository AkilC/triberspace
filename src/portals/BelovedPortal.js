...
const createRippleMaterial = () => {
  const vertexShader = `
    ...
    float combinedNoise(vec2 uv) {
      float noise1 = snoise((uv + vec2(1.0, 1.0)) * vec2(5.0, 5.0) + vec2(time * 0.03, time * 0.05));
      float noise2 = snoise((uv + vec2(-1.0, 1.0)) * vec2(10.0, 10.0) + vec2(time * -0.02, time * 0.08));
      float noise3 = snoise((uv + vec2(1.0, -1.0)) * vec2(2.0, 2.0) + vec2(time * 0.01, time * -0.03));
    ...
    void main() {
      vUv = uv;
      float noiseValue = combinedNoise(vUv * vec2(1.0, 0.5) + vec2(time * 0.02, time * 0.02));
      float displacement = noiseValue * 0.03; // Adjust the value 0.1 to control the displacement amount
  
      // Apply the displacement to the z component of the position
      vec3 displacedPosition = position + vec3(0, 0, displacement);
